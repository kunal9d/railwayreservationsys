...
public class TrainTest {
	...
	public void getTrainTest() {
		...
		.thenReturn(Stream.of(new Train("123","abcd", "kannauj", "Lucknow",60)).collect(Collectors.toList()));
	...
	public void trainByIdTest() {
		String id="123";
		Optional<Train> trainOpt = Optional.of(new Train("1238","raj", "kannauj", "Lucknow",60));
		when(trainRepository.findById(id))
		.thenReturn(trainOpt);
		Train train = trainOpt.get();
		assertEquals(train, trainService.getTrainbyId(id));
	}
	...
	public void deleteByIdTest() {
		...
		//assertNull(id);
		assertNull(trainRepository.findById(id));	
	}
